# Packet 024B — Corpus Boundary and Governance Contract
...
Define the next Milestone 16 planning packet after Packet 024A completed M16 definition and Qdrant evidence reconciliation.

Packet 024B should define the first M16 corpus boundary and governance contract before any Qdrant prototype, chunking implementation, embedding installation, platform mutation, or real corpus indexing occurs.
...
Create a docs-only corpus boundary and governance contract for M16.

The contract should answer:

```text
what content may be indexed in the synthetic prototype corpus;
what content may later be considered for a small approved real corpus;
what content must be excluded by default;
whether docs and vault material should be separated;
how status, authority, review state, supersedence, privacy, freshness, and source kind affect retrieval eligibility;
how current-state and command-center notes are handled;
what approval is required before real corpus indexing;
what must be proven before moving from synthetic corpus to real corpus.
```

024B must not settle chunk-size, point-ID, embedding-model, or gateway tool-name details except where the corpus contract needs metadata requirements. Those belong in 024C and 024D.

## Working M16 definition from 024A
...
024B owns the first phrase: `approved canonical Markdown corpus`.

## Corpus classes to define

024B should classify corpus sources into these families:
...
## Recommended default corpus posture

024B should start from this recommended posture:
...
docs and vault content should remain separable by corpus boundary;
...
The synthetic corpus should be Markdown-derived and deliberately adversarial.

024B should define the synthetic corpus as safe because:
...
The synthetic corpus should be used to test include/exclude policy, not retrieval quality benchmarks.

## Real corpus candidate posture

024B should not approve a real corpus for indexing.

It should define that a later real corpus pilot may include only explicitly selected, approved, non-private, canonical Markdown records.
...
## Docs versus vault separation question

024B should make an explicit recommendation on docs/vault separation.
...
024B should define default retrieval eligibility before Qdrant exists.
...
024B should preserve the conservative rule from prior decisions and M16 research:
...
024B should define a fail-closed privacy rule:
...
This must apply before Qdrant ingestion, not merely at retrieval time.

## Real corpus approval gate

024B should require a later explicit owner approval before real corpus indexing.
...
Without that approval, only synthetic corpus planning/prototype work may proceed in later implementation packets.

## Validation and audit requirements

024B should define validation requirements for later packets:
...
## Expected 024B deliverable

024B execution should create a docs-only result:
...
synthetic trap corpus contract;
real corpus candidate families;
docs/vault separation recommendation;
status/authority/review/supersedence/privacy eligibility rules;
current-state and command-center default exclusion rule;
real corpus approval gate;
open questions carried to 024C/024D;
recommendation for next packet.
```

## Acceptance criteria for 024B execution

024B execution is complete only if:
...
corpus boundaries and exclusions are explicit;
current-state and command-center default retrieval posture is explicit;
real corpus approval gate is explicit;
...
Stop immediately if 024B requires:
...
This draft packet does not authorize 024B execution until explicitly owner accepted.
...
After accepted 024B execution, the likely next packet should be:
